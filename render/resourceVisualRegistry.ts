 ...
 *
 * Body never knows resource ids directly — the consumer registers visuals
 * at app startup via {@link registerResourceVisual}. This keeps body free
 * of consumer-specific resource data while still letting its mesh pipeline
 * tint tiles based on the dominant resource.
 */
...
/**
 * Registers the rendering properties for a resource id. Called by the
 * consumer feature (typically `resources`) at app startup. Safe to call
 ...
 * when no visual has been registered for that id. The mesh pipeline
 * falls back to the biome base color when this returns undefined.
 */
