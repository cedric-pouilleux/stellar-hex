...
export interface BodyRingsConfig {
  ...
  /** When true, `tick()` still tracks positions but stops accumulating spin. */
  paused?:          boolean
  /** Scales spin accumulation (default 1). */
  speedMultiplier?: number
  /** Per-frame world position of the planet (for shadow ray origin). */
...
export interface BodyRingsHandle {
  ...
  /** Live flag — set to pause/resume spin accumulation without rebuilding. */
  setPaused(paused: boolean): void
  /** Live multiplier — spin scales with this value. */
...
export function buildBodyRings(config: BodyRingsConfig): BodyRingsHandle {
  ...
  let speedMult  = config.speedMultiplier ?? 1

  const getPlanetWP = config.getPlanetWorldPos

  function tick(dt: number): void {
    if (!paused) spinAngle += config.rotationSpeed * dt * speedMult
  ...
  function setPaused(next: boolean): void { paused = next }
  function setSpeedMultiplier(mult: number): void { speedMult = mult }
  ...
  return { carrier, mesh, tick, updateVariation, setPaused, setSpeedMultiplier, dispose }
}
