...
import { buildPermTable } from './oceanMask'
...
describe('buildPermTable', () => {
  ...
  it('sampling points on the unit sphere matches createNoise3D output', () => {
    // Sanity check: if our perm table matches, then any CPU simulation that uses
    // createNoise3D(seededPrng(seed)) will produce the same output the GLSL
    // shader sees through uOceanPerm.
    const seed    = 'ocean-check'
