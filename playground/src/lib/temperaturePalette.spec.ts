...
/** Minimal rocky-body builder â€” only the temperature fields matter for the palette. */
...
describe('deriveTemperatureAnchors', () => {
  it('returns the volcanic anchors for a Io-class body (T_avg > 400 Â°C)', () => {
    const a = deriveTemperatureAnchors(rocky(380, 460))
    expect(a.colorLow).toBe('#2a0a02')
    expect(a.colorHigh).toBe('#c44820')
  })

  it('returns the scorched anchors for a Venus-class body (200 < T_avg â‰¤ 400)', () => {
    const a = deriveTemperatureAnchors(rocky(380, 480))
    // T_avg = 430 â†’ still volcanic â€” sanity check the boundary is on T_avg, not on tempMax.
    expect(a.colorHigh).toBe('#c44820')

    const venus = deriveTemperatureAnchors(rocky(420, 470))   // T_avg = 445 â†’ volcanic
    expect(venus.colorHigh).toBe('#c44820')

    const hotEnough = deriveTemperatureAnchors(rocky(400, 420)) // T_avg = 410 â†’ still volcanic
    expect(hotEnough.colorHigh).toBe('#c44820')

    const justBelow = deriveTemperatureAnchors(rocky(380, 410)) // T_avg = 395 â†’ scorched
    expect(justBelow.colorLow).toBe('#3a1808')
    expect(justBelow.colorHigh).toBe('#c08040')
  })

  it('returns the arid anchors for a hot-desert body (50 â‰¤ T_avg < 200)', () => {
    const a = deriveTemperatureAnchors(rocky(0, 200))            // T_avg = 100
    expect(a.colorLow).toBe('#4a3520')
    expect(a.colorHigh).toBe('#d4b478')
  })

  it('returns the temperate anchors for an Earth-like body (-20 â‰¤ T_avg < 50)', () => {
  ...
  it('returns the cold anchors for a tundra body (-80 â‰¤ T_avg < -20)', () => {
  ...
  it('is deterministic â€” same inputs yield the same anchors', () => {
  ...
  it('keys on the mean temperature, not on tempMin or tempMax alone', () => {
    // Wide swing (-200 â†’ +200) â†’ T_avg = 0 â†’ temperate, not glacial nor volcanic.
...
describe('TEMPERATURE_PALETTE_BANDS', () => {
  it('exposes six bands ordered hottest â†’ coldest', () => {
...
describe('deriveLavaColor', () => {
  ...
  it('returns the dark red fallback at and below 100Â°C', () => {
