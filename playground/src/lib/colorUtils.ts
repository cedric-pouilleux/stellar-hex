/**
 * Playground colour helpers — hex → `THREE.Color`, RGB lerp, and
 * temperature-anchored piecewise-linear interpolators used by the
 * playground's composition catalogues (metallic, gas, …).
 *
 * Kept here rather than in the lib because the temperature-keyed
 * interpolators only make sense alongside caller-owned climate /
 * composition tables.
 */
...
export function c(hex: number): THREE.Color { return new THREE.Color(hex) }
 ...
 * Clamps at the first / last anchor when `avgC` sits outside the range.
 */
export function tempLerp(anchors: readonly (readonly [number, THREE.Color])[], avgC: number): THREE.Color {
...
export function numLerp(anchors: readonly (readonly [number, number])[], avgC: number): number {
