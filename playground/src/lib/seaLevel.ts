 ...
 * rescales the translucent liquid sphere in world units. The slider in
 * `LiquidControls` drives a normalised fraction in `[0, 1]`: the fraction is
 * read as a band-space position (`band = fraction * bandCount`), so
 * `0` pins the ocean on the core and `1` floats it above the tallest peak.
 * `HexaPane` converts the band back to world units before forwarding to
 * `body.liquid.setSeaLevel` and mirrors it into the hover tooltip.
 *
